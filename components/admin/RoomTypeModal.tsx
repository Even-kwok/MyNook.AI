...
export const RoomTypeModal: React.FC<RoomTypeModalProps> = ({
  ...
  useEffect(() => {
    if (editingRoomType) {
      setFormData({
        room_key: editingRoomType.room_key,
        display_name: editingRoomType.display_name,
        description: editingRoomType.description || '',
        display_order: editingRoomType.display_order,
        is_active: editingRoomType.is_active,
      });
    } else {
      setFormData({
        room_key: '',
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-slate-200">
          <h2 className="text-2xl font-bold text-slate-900">
            {editingRoomType ? '编辑房间/场景类型' : '添加房间/场景类型'}
          </h2>
